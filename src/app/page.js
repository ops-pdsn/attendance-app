...
export default function Home() {
  ...
  // FIX 1: Use useCallback for fetchData to satisfy ESLint
  ...
  // FIX 2: Close ALL dropdowns on scroll (solves the floating dropdown issue)
  useEffect(() => {
    ...
    // Listen for scroll on window and any scrollable container
    window.addEventListener("scroll", closeAllDropdowns, true);

    // Also close on resize (viewport changes)
  ...
  const applyQuickFilter = (preset) => {
    ...
    setShowFilterDropdown(false);
    toast.info(
      `Filter applied: ${preset.replace(/([A-Z])/g, " $1").toLowerCase()}`
    );
  };
  ...
  const handleMarkAttendance = async (attendanceData) => {
    try {
      ...
      if (res.ok) {
        toast.success(
          `Attendance marked: ${
            attendanceData.status === "office" ? "🏢 Office" : "🚗 Field"
          }`
        );
        fetchData();
      } else {
  ...
  const handleCarryForward = async (fromDate) => {
    const confirmed = await confirm({
      ...
      message:
        "This will copy all incomplete tasks from this date to tomorrow. Continue?",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-300">
          ...
          <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl border border-white/20 dark:border-slate-700/50 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 relative overflow-hidden">
        ...
        {dateFilter.start && dateFilter.end && (
          <div className="mb-4 px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl flex items-center justify-between animate-fadeIn">
            <span className="text-sm text-blue-700 dark:text-blue-300 flex items-center gap-2">
              ...
              Showing: {new Date(dateFilter.start).toLocaleDateString()} -{" "}
      ...
      {showOrgTree && <OrgTree onClose={() => setShowOrgTree(false)} />}
